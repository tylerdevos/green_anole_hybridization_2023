...
#filter based on call rate, keeping only SNPs with data for at least 95% of samples
vcffilter -f "NS > 95" all.biallelic.snps.MQ20.dp4.fixed.vcf > all.biallelic.snps.MQ20.dp4.95d.vcf

#filter based on minor allele frequency, keeping SNPs with maf higher than 3%
grep -v "AF=-nan" all.biallelic.snps.MQ20.dp4.95d.vcf | vcffilter -f "AF > 0.03 & AF < 0.97" > all.biallelic.snps.MQ20.dp4.95d.maf3.vcf

#quantify missing data
vcfkeepgeno all.biallelic.snps.MQ20.dp4.95d.maf3.vcf GT | grep -v "#" | cut -f 10- > all.geno.txt
...
vcfsamplenames all.biallelic.snps.MQ20.dp4.95d.maf3.vcf > all_samples.txt
...
do
samples=$line
vcfkeepsamples all.biallelic.snps.MQ20.dp4.95d.maf3.vcf $samples > all.biallelic.snps.MQ20.dp4.95d.maf3.70perc_data_samples.vcf
done
